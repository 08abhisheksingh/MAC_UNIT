...
The OpenLane flow was executed on an Azure Virtual Machine, which operates in a command-line environment without graphical user interface support. Consequently, GTKWave could not be launched directly for waveform visualization. To address this limitation, the simulation-generated VCD (Value Change Dump) file was exported from the Azure VM and analyzed using a compatible waveform visualization tool. This approach enabled comprehensive functional verification of the MAC unit while maintaining the advantages of a cloud-based simulation environment.
